...
(function(root, factory) {
	...
	Ssi_upload.prototype.init = function(element) {
		...
		var $uploadBtn = $('<button id="ssi-uploadBtn" class="ssi-button success ssi-hidden" >' +
			'<span class="ssi-btnIn">' + "上传" + '&nbsp;</span>' +
			'<div id="ssi-up_loading" class="ssi-btnIn"></div></button>');
		var $clearBtn = $('<button id="ssi-clearBtn" class="ssi-hidden ssi-button info" >' + "清除" +
			'</button>');
	...
	Ssi_upload.prototype.toUploadFiles = function(files) {
		...
		if(typeof this.options.maxNumberOfFiles === 'number') {
			if(files.length > this.options.maxNumberOfFiles) {
				alert("最多只选择" + this.options.maxNumberOfFiles + "张");
				return;
			}
			...
			if(thisS.options.preview) {
				var getTemplate = function(content) {
					return '<table class="ssi-imgToUploadTable ssi-pending">' +
						...
						'<tr><td><button data-delete="' + index + '" class=" ssi-button error ssi-removeBtn"><span class="trash10 trash"></span></button><span class="strWH"></span></td></tr>' +
						'<tr><td class="td-fileName">' + cutFileName(filename, ext, 15) + '</td></tr></table>'
				};
...
//图片上传文件分辨率检查
var checkImage = function(isAlert) {
	var imgCode = "1024x1024,2048x2048,720x1280,1080x1920,1440x2560"; // 上传要求的分辨率
	var alertMsg = "";
	$.each($("table.ssi-imgToUploadTable"), function() {
		var img = $(this).find('.ssi-imgToUpload');
		var fileName = $(this).find('td.td-fileName');
		var strWH = img[0].naturalWidth + "x" + img[0].naturalHeight;
		var style = "";
		if(imgCode.indexOf(strWH) == -1) {
			//alertMsg += "已自动剔除【" + fileName.html() + "】的【" + strWH + "】！\n";
			//			$(this).find('td button.ssi-removeBtn').click();
			style = "style='color:#333;font-size:10px;'";
			$(this).find('td span.strWH').html("<span " + style + ">&nbsp;" + strWH + "</span>")
		} else {
	...
	Ssi_upload.prototype.uploadFiles = function() { // upload the pending files
		if(this.pending > 0) {
			//上传停止按钮去掉
			this.$element.find('#ssi-abortBtn').addClass('ssi-hidden');
			//上传100%去掉
			this.$element.find('#ssi-uploadBtn').addClass('ssi-hidden');
			...
			if(typeof this.options.beforeUpload === 'function') {
				try {
					...
					// debugLog('There is an error in beforeUpload callback');
					// debugLog(err);
					thisS.abortAll();
					return;
				}
		...
		function ajaxLoopRequest(formData, ii) {
			...
			var ajaxOptions = $.extend({}, { //store the request to the uploadList variable
				beforeSend: function(xhr) {
					...
					thisS.uploadList[ii].withCredentials = true;
					...
					if(typeof thisS.options.beforeEachUpload === 'function') {
						try {
							...
							// debugLog('There is an error in beforeEachUpload callback');
							// debugLog(err);
							thisS.abortAll();
							return;
						}
		    	...
		    	success: function(rsp) {
					...
					if(rsp.msg === "success"){
						// 上传成功后隐藏清除按钮
						thisS.$element.find('#ssi-clearBtn').addClass('ssi-hidden');
						// 上传成功后隐藏停止按钮
						thisS.$element.find('.ssi-abortUpload').addClass('ssi-hidden');
						layer.msg("上传成功",{icon:1,time:3000})
					    $("#"+rsp.id+"_url").val(rsp.path);
						$("#"+rsp.id).attr('src',rsp.path);
						$("#"+rsp.id).val(rsp.viewImage);
						$("#"+rsp.id+"_viewImage").val(rsp.path);
						if(rsp.id == "passive_url"){
							$("#passiveViewBtn").show();
						}else{
							$("#activeViewBtn").show();
						}
					...
					if(error !== 'abort') {
						...
						if(typeof thisS.options.onEachUpload === 'function') { //execute the onEachUpload callback
							try {
								...
								// debugLog('There is an error in onEachUpload callback');
								// debugLog(err);
							}
						}
						...
						// debugLog(arguments); //log the error
						// debugLog(" Ajax error: " + error + "哦");
					}
				}
	...
	var finishUpload = function(thisS) { //when every uplaod ends
		...
		if(typeof thisS.options.onUpload === 'function') {
			try {
				...
				debugLog('There is an error in onUpload callback');
				debugLog(err);
			}
		}
	...
	$.fn.ssi_uploader = function(opts) {
		...
		return this.each(function() {
			...
			if($element.is('input[type="file"]')) {
				...
				debugLog('The targeted element is not file input.')
			}
	...
	var locale = {
		...
		cn: {
			...
			clear: '清空',
			drag: '', // 支持拖拽的提示语
			sizeError: '文件\n$1\n超过$2', // $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
			extError: '不支持 $1 类型', //$1=file extension ie(exe files are not supported)

    ...
    methods: {
        ...
        del_record(id){            
            if(confirm("确定要删除此记录吗？")){
                jquery_ajax(ACTION_URL.departments_delete,"post",id,true,()=>{
                    alert("操作成功");
        ...
        submit_form:function () {                        
            ...
            jquery_ajax(ACTION_URL.departments_modify,"post",this.form_data,true,(json_result)=>{                
                ...
                if(this.form_data.id > 0){
                    location.href="/production/department/department_new.html?current_page="+this.search_param.page;
                }else{
                    location.href="/production/department/department_new.html";
                }
            });                    
        },        
        load_edit_data(){ //拉取修改页的数据            
            jquery_ajax(ACTION_URL.departments_detail,"post",this.form_data.id,false,(json_result)=>{

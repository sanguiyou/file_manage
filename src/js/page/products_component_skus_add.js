var vue_instance = new Vue({
    ...
    methods: {
        submit_form:function () {                    
            ...
            jquery_ajax(ACTION_URL.component_skus_modify,"post",this.form_data,true,(json_result)=>{                
                console.log(json_result);
                alert("操作成功");
                if(this.form_data.id > 0){
                    //location.href="/production/product/products_component_skus.html?current_page="+this.current_page;
                }else{
                    location.href="/production/product/products_component_skus.html";
                }
            });                    
        },        
        load_edit_data(){ //拉取修改页的数据            
            jquery_ajax(ACTION_URL.component_skus_detail,"post",this.form_data.id,false,(json_result)=>{
